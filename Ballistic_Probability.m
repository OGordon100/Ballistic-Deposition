...
lat_x = 30;             % -2 for buffer zone around matrix
lat_y = 300;            % Must be at least extensionthresh
extensionthresh = 100;
Nblockmax = 50000;      % Number of blocks to deposit
repeats = 2;            % Number of repeats to run
stickyness = 60;        % Percentage probability of sticking

activefinder = zeros(1,3,'int8');
...
oldfolder = pwd;
mkdir(['x=',num2str(lat_x-2),' stick=',num2str(stickyness)]);
cd(['x=',num2str(lat_x-2),' stick=',num2str(stickyness)]);
...
for rep = 1:repeats
    ...
    for N = 1:Nblockmax
       ...
       %dropx=input('Drop Point: ');  % For manual dropping to test
        ...
        [~,activefinder] = max(activelattice(heightrange,dropx-1:dropx+1)); % change heightrange?
    ...
    % Calculate variance for all runs
    ...
    FigHandle1.Name=['Ballistic Probability, Rep = ',num2str(rep)];
   ...
   %drawnow               % Uncomment to get all graphs/plot in realtime
   %map = [0,0,0;1,0,0;1,1,0;];
   %colormap(map);
